...
from MAEImputer2 import ReMaskerStep
import numpy as np
...
train_df = train_df.loc[:, ~train_df.columns.str.contains("time", case=False)]
val_df = val_df.loc[:, ~val_df.columns.str.contains("time", case=False)]
df_test = df_test.loc[:, ~df_test.columns.str.contains("time", case=False)]
...
columns = train_df.shape[1] - 3 # + 3 because of: first_race, chartyear, hadm_id

mask_ratio = 0.25
max_epochs = 400
save_path = 'ReMask_0.25Mask_L_No_time'
...
imputer = ReMaskerStep(dim=columns, mask_ratio=mask_ratio, max_epochs=max_epochs, save_path=save_path, batch_size=batch_size,
                      ...
                      weigths=weigths, use_remask=True)
